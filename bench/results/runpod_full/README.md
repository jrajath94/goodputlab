...
- `bench/figures/runpod_full_ttft_chat.png` — TTFT vs arrival rate, one
  line per topology (chat only; regenerated by `make figures` from the
  reconciled cells in this directory). DISAGG / DISAGG_TIER lines are
  absent because the sweep never reached those topologies — see
  "Coverage" above.
